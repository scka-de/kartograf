...
| GitHub Actions CI | `.github/workflows/ci.yml` installs `.[full,dev]` on Python 3.11 and runs `scripts/pre_submission_check.sh` on pushes to `main` and pull requests. Remote run `25401510049` passed on `main`. | Passed |
...
| CI workflow | `.github/workflows/ci.yml` | Runs the non-mutating pre-submission check in GitHub Actions for `main` and pull requests; remote run `25401510049` passed. | Done |
